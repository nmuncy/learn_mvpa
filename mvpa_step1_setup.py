...
def func_detrend(tmp_dir, decon_str, tmp_list):
    ...
    h_cmd = f"""
        cd {tmp_dir}
        3dTcat -prefix tmp_{decon_str}_cbucket -tr {len_tr} "{decon_str}_cbucket_REML+tlrc[0..{len_right}]"
...
# %%
# def func_job(subj, subj_dir, len_tr, task_dict, beh_dur, der_dir):
"""
Step 1: Detrend

Extract TENT sub-bricks
"""
# For testing
subj = "sub-005"
...
for phase in task_dict:
    if type(task_dict[phase]) == list:
        ...
        if not os.path.exists(
            ...
            func_detrend(subj_dir, decon_str, task_dict[phase])
        ...
        for decon in task_dict[phase]:
            ...
            if not os.path.exists(
                ...
                func_detrend(subj_dir, decon_str, task_dict[phase][decon])
...
for count, phase in enumerate(task_dict):
    ...
    for cond in cond_list:
        if not os.path.exists(os.path.join(subj_dir, f"tmp_tf_{phase}_{cond}_r01.1D")):
            h_cmd = f"""
...
#     # print(args.h_sub, args.h_dir, args.h_trl, h_task_dict, args.h_beh, args.h_der)
#     func_job(
#         args.h_sub,
#         args.h_dir,
#         float(args.h_trl),
